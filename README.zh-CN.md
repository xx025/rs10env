...
[空间几何与精确搜索实验](docs/SPATIAL_RESEARCH.md)：按残留格到矩形的距离重建区域，在独立 20 局上仅提高 0.05 格，暂无更强证据。5 个开发棋盘固定前缀后的最后 8 步已被精确证明最优，但这不代表整局最优；140 分目标仍未达到。

进一步实验：[混合邻域重建搜索](docs/NEIGHBORHOOD_RESEARCH.md)。`hybrid_search` 在 40 局新棋盘上平均 **133.50 格**，同棋盘种群搜索为 129.825 格；预热后平均整局 8.887 秒、最长 9.014 秒，无超时。**仍未达到平均 140 格目标**，不以单个高分棋盘或较容易的测试子集代替整体结果。

后续实验：[远程 NRPA 类自适应算法研究记录](docs/REMOTE_RESEARCH.md)。已在指定远程机器上比较单层、双层自适应搜索；30 局独立配对评测为 127.70 格，对照种群搜索 125.53 格，整局约 8.75 秒。**尚未达到平均 140 格的目标**。实验策略 `adaptive_search` 可通过工厂创建，暂不作为默认推荐；不同棋盘集的均分不可直接比较。
